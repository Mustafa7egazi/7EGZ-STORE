...
async function handleLoader() {
  ...
  for (let product of products) {
    ...
    showInfo.addEventListener("click", function (e) {
      e.preventDefault();
      let product = JSON.parse(decodeURIComponent(this.dataset.product));
      window.location.replace(`/7EFZ-STORE/pages/product-info.html?id=${product.id}`);
    });
...
function addToCart(product) {
  ...
  if (window.location.href.includes("home.html")) {
    ...
    cartIcon.addEventListener("click", () => {
      ...
      if (items) {
        window.location.replace("/7EGZ-STORE/pages/cart.html");
      } else {
        let warningModal = new Modal({
  ...
  if (listOfFilteredProducts.length > 0) {
    for (let product of listOfFilteredProducts) {
      ...
      showInfo.addEventListener("click", function (e) {
        e.preventDefault();
        let product = JSON.parse(decodeURIComponent(this.dataset.product));
        window.location.replace(`/7EGZ-STORE/pages/product-info.html?id=${product.id}`);
      });
